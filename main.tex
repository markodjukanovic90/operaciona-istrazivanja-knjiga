...
\end{enumerate}                                                       
 ...
 \fixme{Marko, ovaj dio sa početka sam malo više mijenjao, pa pogledaj da li ti je prihvatljivo.}
...
\end{array}\right)$$
 ...
 U ovom odjeljku ćemo objasniti još jednu korisnu dekompozicionu tehniku za rješavanje problema linearnog programiranja -- \emph{Dantzig-Wolfe} (DW) dekompoziciju. Ova tehnika je originalno dizajnirana za rješavanje problema linearnog programiranja velikih dimenzija. Osnovna ideja DW dekompozicije je da se na podskup ograničenja polaznog problema primijeni \emph{Teorema Minkovskog}, odnosno \emph{Teorema o reprezentaciji za konveksne poliedre}, te da se polazni problem razbije na   više manjih potproblema.
...
Da bi se odredili   koeficijenti doprinosa za $\lambda_j$, odredimo šta se dobija u odgovarajućoj koloni. U funkciji cilja, imamo $c^Tx^{(j)}$, u ograničenjima $A x^{(j)} $ i još jednu jedinicu (na osnovu ograničenja $\sum_{j\in I}\lambda_j = 1$). Tako, kolona na koju se fokusiramo da bismo izračunali doprinose izgleda ovako
 ...
 Sada rješavamo \emph{Column generation potproblem} (CG), čiji je cilj da se pronađe najmanji koeficijent doprinosa. Ukoliko je on manji od nule, dobija se nova kolona koja treba da se doda u potproblem RMP.  U slučaju da je on nenegativan, tada je optimalno rješenje RMP ujedno i optimalno rješenje MP.
...
\end{center} 
  ...
  \vspace{5mm}
   \emph{Iteracija 3.} Prema tome, novi CG potproblem je dat sa 
  \begin{align*}
...
\emph{Iteracija 4.} Na osnovu vrijednosti dualnih promjenljivih, formira se novi CG potproblem
