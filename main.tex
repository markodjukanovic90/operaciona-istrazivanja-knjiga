...
Bazne promjenljive sada lako prepoznajemo iz Tabele~\ref{tab:simpleks_tabela2}, jer odgovarajući vektori kolone zajedno formiraju jediničnu matricu. Prema tome, 
 nebazne promjenljive su $x_2$ i $s_2$, koje dobijaju vrijednost 0, dok ostale lako izračunavamo, odakle je $ x = (4, 0, 5, 0)$. Ponavljamo prethodne korake nad novom simpleks tabelom. Potrebno je pronaći novi pivot element. Lako se vidi da jedino kolona 2 odgovarajuća. Pošto je $5 / (5/2) < 4 / (1/2)$, zaključujemo da  pivot element pronalazimo u prvoj vrsti. Dakle, pivot je 
 ...
 \end{table}
 Kako su sve promjenljive doprinosa manje (ili jednake) 0, zaključujemo da smo pronašli optimum koji je jednak $x = (3, 2, 0, 0)$, a optimalna vrijednost je $f = 5$.
 ...
 Analizirajmo još neke aspekte u ovom primjeru. Na Slici~\ref{fig:simplex_region}, tačke $A, B, C$, i $D$ su bazna dopustva rješenja. Tačke $E$ i $F$ su takođe bazna rješenja, ali nisu dopustiva. Simpleks algoritam prvo kreće od tačke $A$ prema tački $D$, te završava u tački $C$ (optimum).
 \fxnote{Marko, ovdje se ne poklapa, valjda je simpleks krenuo od (0,0), pa presao u (4,0) i na kraju zavrsio u (3,2). Trebalo bi, dakle od tacke D, prema tacki C, pa do tacke B}
...
$$\begin{array}{cccc| c} 
	   ...
	   -1 & 3  & 0 & 0 & 0  
\end{array}$$
\fxnote{Marko, je li problem maksimizacije ili minimizacije? Ovo sad meni nije jasno}
Kako rješavamo problem minimizacije, tražićemo pivota u koloni 2. Pivot je element 
$\overline{a}_{1, 2} = 1$. Pivotiranjem oko tog elementa dobijamo novu simpleks tabelu 
...
Grafička reprezentacija dopustivog, neograničenog regiona, prikazana je  Slici~\ref{fig:unbounded-example-solve}. 
...
$$\begin{array}{ccccc|c}
     -1 & 1 & 1 & 0 & 0 & 3 \\
      ...
      2 & 3 & 0 & 0 & 0 & 0 \\
\end{array}$$
Pretpostavimo smo  za baznu promjenljivu izabrali $y$, tj. pivotiramo oko nekog od elemenata u koloni 2. Za pivota odaberemo $\overline{a}_{1,2}=1$ element, te nakon pivotiranja dobijamo novu simpleks tabelu:
$$\begin{array}{ccccc|c}
	-1 & 1 & 1 & 0 & 0 & 3 \\
    -1 & 0 & 2 & 1 & 0 & 8 \\
    7 & 0 & -6 & 0 & 1 & 0 \\ \hline
    5 & 0 & -3 & 0 & 0 & -9 \\
\end{array}$$
\fxnote{Marko, ovdje je Milana primijetila da u prvoj koloni ima broj 7 koji je pozitivan. Kako onda nema elemenata koji su strogo veci od nule (njena primjedba oznacena crvenom bojom) Nisam dalje citao ovaj primjer, na ovo se moramo zajednicki vratiti}
...
Ako postoji jedinična podmatrica $I_m$ u gornjoj lijevoj blok matrici simpleks tabele, onda je jasno da je odgovor na prvo pitanje pozitivan.
 \fxnote{dragan: u primjeru 4.1}
 Linearnom programu koji sadrži ograničenja $Ax \leq b, x\geq 0$ se dodaju izjednačavajuće varijable da bi uslovi nejednakosti u ograničenjima postali potrebne jednakosti. Na taj način se i formira ova jedinična matrica. 
 
 
 Međutim, šta ako je linearan program direktno dat u kanonskom obliku, tj. sa ogarničenjima oblika $Ax = b, x \geq 0$? Bez smanjenja opštosti, pretpostavimo da je $b \geq 0$. Ako ne postoji očigledno bazno dopustivo rješenje ovog problema, onda uvodimo \emph{umjetne} varijable $s = (s_1,\ldots, s_m)$ nakon čega treba da riješimo sljedeći problem LP-a:
 \begin{align}
