...
\item % http://courses.ieor.berkeley.edu/ieor151/lecture_notes/ieor151_lec13.pdf
   ...
   Neka je u ulazu dat skup od $V$ poslova koji treba da budu urađeni na mašini. Svaki posao $j$ je dat intervalom $[r_j, d_j]$, $r_j < d_j$, gdje $r_j$ označava dopustivo vrijeme puštanja posla u rad (na mašini) dok $d_j$ označava vrijeme do kada se posao mora završiti.  Vrijeme procesiranja posla $j$ je dato sa $d_j$. 
   ...
   \end{align*} 
    ...
    ($a$) uz pomoć Cplex-a; ($b$) uz pomoć PuLP modula. 
    \item Neka je instanca problema data sa $n$ brojeva iz skupa $\mathbb{R}^p$, $p\in \mathbb{N}$. Sa $w_{i}=(w_{i1},\ldots, w_{ip})$ označimo $i$-ti element u skupu $S$. 
