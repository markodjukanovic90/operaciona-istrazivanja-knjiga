...
\begin{align}
	...
	& x, s \geq 0. \nonumber
\end{align}
Ovo nije linearni program, jer trće ograničenje  nije linearno.
...
\end{array} \right ) 
$$
Prema tome, u tački $(x,y,s)$, Njutnov pravac (pomjeraj)
$
\begin{pmatrix}
	\bigtriangleup  x  \\
	\bigtriangleup  y  \\
...
\end{align}
 ...
 \begin{itemize}
 	\item $x_1$ i $x_2$ su binarne varijable. Uvedemo smjenu $y=x_1 x_2$. Sljedeći uslovi prisiljavaju da $y$ primi vrijednost proizvoda:
 	  \begin{align*}
 		      &y \leq x_1 \\
 		      &y \leq x_2 \\
 		      & y \geq x_1 + x_2 - 1 \\
 		      & y \in \{0, 1\}.
 	  \end{align*}
   \item $x_1$ je binarna, $x_2$ je realna (pozitivna) varijabla, $0 \leq x_2 \leq u$. opet uvodimo istu smjenu $y= x_1 x_2$ koja je neprekidna, te dodamo sljedeća ograničenja da prisilimo $y$ da primi vrijednost proizvoda:
    \begin{align*}
    	 &y \leq u x_1 \\
    	 &y \leq x_2 \\
    	 & y \geq x_2 + u(1-x_1) \\
...
Sada posmatrajmo dualni problem potproblema (REFF), ignorišući term $g^T\overline{y}$ koji je konstantan.
\begin{align}
	&\max u^T (c-B\overline{y})\\
	&s.t. u^TA\leqslant f\\
	&u\geqslant 0\label{eqn:dd}
\end{align}

Sada uključujemo teoriju dualnosti i razmatramo sljedeće.
Ako je optimalno rješenje problem REFF $z^*$, a optimalno rješenje duala $u^*$, na osnovu teoreme o jakoj LP dualnosti imamo

Jednačina 4.14

Dalje, uočiomo da oblast dopustivih rješenja dualnog problema REFF ne zavisi od vrijednosti $\overline{y}$. Stoga, $u^*$ je uvijek dopustivo rješenje problema REFF, vez obzira na vrijednost $\overline{y}$. Odavde, na osnovu slabe LP dualnosti dobijamo

Nejednakost 4.15

Posljednja nejednakost propisuje jednju donju granicu funkcije cilje polaznog problema. Ovo je ključni element koji se koristi u Benderovoj dekompoziciji, pomoću kog se određuje jedna odsjecajuća ravan za master problem (koji će biti definisan kasnije). Treba napomenuti da odsjecajuća ravan REFF ukazuje na činjenicu da, ako bi se za y uzela vrijednost $y=\overline{y}$, funkcija cilja bi ... NEJASNO

Ovo takođe daje i donju granicu za druge vrijednosti $y$ koje bismo mogli probati.

Posmatrajmo sada slučaj da je dualni potproblem neograničen. U toj situaciji baži nejednakost

Nejednakost 4.16

gdje je ${(u^*)}^T$ extreme ray solution.

U ovom slučaju, cilj je eliminisati ovaj extreme ray iz daljeg razmatranja (budućih rješenja). Da bi smo to uradili, potrebno je osigurati da izraz IZRAZ manji ili jednak od nule u svim narednim koracima. Tako, master problemu dodajemo novu odsjecajuću ravan, definisanu sa
...
Upisivanje modela u .lp format se vrši pozivom funkcije \emph{writeLP()}, čiji je argument naziv fajla. 
 ...
 \end{verbatim}
 \section{Primjena rješavača PuLP na rješavanje jednog Problema Lokacija}%Capacitated Facility Location
