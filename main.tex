...
na kanonski oblik pogodan simpleks metodi $(A {x} = b)$ dodavanjem ``labavih'' varijabli $s$ na sljedeći način:
...
\end{equation}
 ...
 \chapter{Tehnike rješavanja Cjelobrojnog programiranja}
...
\begin{algorithm}[H] 
	\begin{algorithmic}
		...
		\STATE  $v_j \gets$ Neki od aktivnih čvorova;
...
U ovom primjeru posmatrajmo 0-1 problem ruksaka. Ponovimo da nam je, kod ovog problema, na raspolaganju određen broj proizvoda, od kojih svaki proizvod ima svoju vrijednost i težinu. Ruksak je ograničenog maksimalnog kapaciteta (težine) $C$. Podsjetimo se i da se u 0-1 problemu ruksaka svaki proizvod ili ne uzima nikako ili se uzima čitav. Cilj je, kao i u drugim problemima ruksaka, odabrati onaj skup proizvoda, tako da je njihova ukupna vrijednost maksimalna, a da se ne naruši kapacitet ruksaka.
...
s.t.
$$\sum_{i=0}^{n-1}x_iw_i\leqslant C$$
$$x_i\in\{0,1\}, i = 1,2,...,n$$
...
Treba napomenuti da se u rješavanju velikog broja problema kombinatorne optimizacije pomoću genetskih algoritama (ali i drugih heurističkih metoda) često dozvoljava rad sa nedopustivim rješenjima. Razlog za to leži u činjenici sva nedopustiva rješenja ``podjednako loša'', a nekada je lakše popraviti nedopustivo rješenje u dopustivo rješenje visokog kvaliteta, nego vršiti pretragu samo na prostoru dopustivih rješenja slabijeg kvaliteta.  Zbog toga se za prevazilaženje pojave nedopustivih rješenja najčešće koristi jedna od sljedeće dvije strategije: 
\begin{itemize}
	\item
	...
	\item uvodi se tzv. kaznena funkcija, (engl. penalty function), kojom se ``kažnjavaju'' nedopustiva rješenja (najčešće smanjenjem vrijednosti funkcije prilagođenosti), ali se ona ne izbacuju iz razmatranja, u nadi da perspektivnija nedopustiva rješenja mogu prerasti u kvalitetna dopustiva rješenja.
\end{itemize}

U našem primjeru, pojavu nedopustivih rješenja, dakle, možemo riješiti automatskom dodjelom minimalne vrijednosti funkcije prilagođenosti, ili ``kažnjavanjem'' takvih rješenja smanjenjem vrijednosti funkcije fitnesa. Neke od strategije za ``kažnjavanje'' nedopustivih jedinki bi mogle biti zasnovane na procjeni broja proizvoda koje treba zamijeniti (ili izbaciti) iz datog rješenja, a da rješenje postane dopustivo.

Računanje funkcije prilagođenosti: Za posmatrani problem, najprirodnija fukcija prilagođenosti bi se mogla definisati na način da ona bude jednaka ukupnom zbiru vrijednosti proizvoda uključenih u rješenje (pod uslovom da je to rješenje dopustivo), a da je vrijednost funkcije prilagođenosti  nedopustivih rješenja   jednaka nuli. 
...
Operato ukrštanja: Kao što je već pomenuto, osnovni operatori ukrštanja su tzv. jednopoziciono i dvopoziciono ukrštanje. Na našem primjeru ćemo objasniti  jednopoziciono ukrštanje, koje bi moglo biti realizovano na sljedeći način:
\begin{figure}
...
Operator selekcije: TODO



Višedecenijska primjera genetskih algoritama u rješavanju velikog broja problema kombinatorne optimizacije dovela je do pojave velikog broja različitih varijanti algoritma, koje se, između ostalog, razlikuju po operatorima koji se primjenjuju, načinu formiranja početne populacije, primjenama različitih strategija lokalnog pretraživanja itd. 

Jedan od korisnih aspekata koji u značajnoj mjeri može da ubrza rad (osnovnog) genetskog algoritma je koncept keširanja. 

Kao što je poznato, keširanje se koristi u situacijama kada se dio podataka koji se obrađuju smješta u zaseban memorijski prostor, uz pretpostavku da će se ti podaci ponovo koristiti u kasnijim fazama izvršenja programa. Na taj način se budući zahtjevi za tim podacima opslužuju brže, u odnosu na vrijeme njihovog ponovnog izračunavanja. 

Koncept keširanja se u genetskim algoritmima može iskoristiti u situacijama kada je, u različitim iteracijama, moguća pojava  identičnih jedinki. S obzirom da je računanje funkcije cilja vremenski često zahtjevna operacija, poželjno je primijeniti tehniku kojom se pamte genetski kodovi i njihove vrijednosti, pa ih kasnije koristiti, nego ih uvijek iznova računati.  Keš se može implementirati na različite načine, od kojih je najprirodnija implementacija preko heš-reda (engl. hash-queue) strukture podataka, unaprijed zadanog kapaciteta. Kada treba da se računa vrijednost funkcije cilja za datu jedinku, prvo se provjerava da li je ta vrijednost već izračunata i pohranjena u kešu. U slučaju da vrijednost ne postoji u keš strukturi, ona se tada po prvi put računa i pohranjuje u keš. U slučaju da je vrijednost pronađena, ona se ne računa, već se samo pročita iz keša. U slučaju da se keš napuni, brišu se one vrijednosti koje najduže nisu korištene (engl. least recently used - LRU). Treba istaći da keširanje ni u kom slučaju ne utiče na kvalitet rješenja, već samo služi za ubrzanje izvršenja genetskog algoritma.



Naprednija šema  genetskog algoritma, koja sadrži i koncept keširanja je prikazana na Slici \ref{fig:gascheme1}.


\begin{figure}[htp]
	\centering
	\includegraphics[width= 0.8\textwidth]{gasrp.eps} \caption {Šema GA algoritma sa uključenom tehnikom keširanja}\label{fig:gascheme1}
\end{figure}
