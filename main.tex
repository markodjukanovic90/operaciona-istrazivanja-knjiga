...
\subsection{Heuristički Metodi}
 ...
 CPLEX alat je generalni solver za rjesavanje optimizacionih problema koji ukljucuju i LP. Konstruisan je od strane IBM-a, i moze da se skine sa linka \url{https://www.ibm.com/products/ilog-cplex-optimization-studio}. On je dio IBM ILOG CPLEX Optimization Studio-a koij ukljucuje i druge opcije poput \emph{Constraint Programming} solvera itd.
 
 U nastavku opisujemo proces kodiranja modela u CPLEX-u u programskom jeziku C++.
 Da bismo ukljucili CPLEX u C++ kod, nepohodno je ukljuciti cplex u C++ program sa:
 \begin{verbatim}
 	#include<ilcplex/ilocplex.h>
 	ILOSTLBEGIN
 \end{verbatim}
 Da bi se inicijalizovao LP model, neophodno je prvo inicijalizovati ILO okrizenje. Taj objekat pripada klasi IloEnv i deklarise se sa: 
 \begin{verbatim}
 	IloEnv env;
 	try{
 ...
 \noindent Unutar try-catch bloka formirao prvo model (objekat klase IloModel), a zatim se model povlaci cplex-om (kreira se objekat klase IloColex) i rjesava pozivanjem odgovarajuce metode.
 Model se kreira iz klasee IloModel ciji konstruktor uzima objekat klase IloEnv (tj. $env$) kao parameter. 
 
 
 \begin{verbatim}
 	IloModel model(env); //def. modela
 	// potrebno deklarisati gaol funkciju, varijable i uslove i dodati u model
 ...
 \noindent Objasnimo sada kako deklarisati funkciju cilja, varijable i uslove u modelu. 
 
 \noindent \textbf{Deklarisanje varijabli}. Ako zelimo da deklarisemo varijablu $x$ gdje je $0 \leq x \leq 40$, koristimo kalsu IloNumVar, te odgovara sljedeca linija koda:
 \begin{verbatim}
 	IloNumVar x = IloNumVar(env,0.0,40.0);
 \end{verbatim}
 
 ako zelimo da definisemo varijablu koja je cjelobrojna, recimo binarna varijabla, onda je:
 \begin{verbatim}
 	IloNumVar x = IloNumVar(env,0.0, 1.0, ILOINT);
 \end{verbatim}
 
 \noindent Ako je potrebno definisati vektor promjenjivih $x=(x_1,...,x_n)$, gdje je recimo $x_i \geq 1$, onda se definise objekat klase IloNumVarArray, tj.:
 \begin{verbatim}
 ...
 \noindent Ako zelimo da deklarisemo visedimenzionalne promjenjive kao sto he $x_{i,j}, i=1,...,n$ i $j=1,...,m$, onda koristimo templejt $\textbf{IloArray}<>$.
 
 \begin{verbatim}
 	typedef IloArray<IloNumArray> FloatMatrix; // x_ij 
 	typedef IloArray<FloatMatrix> Float3Matrix; // x_ijk
 ...
 \noindent \textbf{Funkcija cilja}. 
 Postoje dvije klase IloMaximize, IloMinimize za deklarisanje objekta koji odgovara
 funkciji cilja. IloMaximize uzima dva parametra objekat IloEnv okruzenja, te objekat klase 
 IloExpr gdje se definise linearni izraz koji odgovara funkciji cilja. 
 ...
 \noindent Deklarisanje izraza $expr$ ide pomocu operatora ``+=,'' gdje se na desnoj strani nalazi objekat IloNumVar koji predstavlja varijablu. Varijabla moze da bude pomnozena nekim skalarom. 
 Recimo, ako zelimo da implementiramo funkciju cilja $\sum_{i=1}^n c_i x_i$, kod bi bio ovakav:
 ...
 \noindent \textbf{Dodavanje uslova u model}. Uslovi se mogu direktno dodati kao 
 model.add($expr1 <= expr2$), gdje mjesto ``$<=$'' mogu biti ``=='' ili ``$>=$''. Mjesto $expr1$ ili $expr2$ moze da bude i skalar. Recimo da treba da modelujemo 
 $$s_j = \sum_t=1^n t \cdot x_{jt}, \forall j.$$ 
 Sljedeci kod implementira ovaj uslov:
 \begin{verbatim}
 	for (j = 0; j < N; j++) {
 		IloExpr expr(env);
 		for (t = 1; t <= n; t++)
 ...
 \noindent \textbf{Metode za ispis rjesenja (statistika)}. Metod getStatus  pokazuje da li je rjesenje koje je nadjeno dopustivo, ograniceno ili optimalno ili je cplex pokazao da je model nedopustiv ili ne-ogranicen.
 ...
 \end{verbatim}
 \subsection{Rješavač SciPy.PuLP u Python-u}
