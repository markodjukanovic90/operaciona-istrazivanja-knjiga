...
\emph{Koliko treba konzumirati pojedinih namirnica da bi se zadovoljila dnevna
...
U nastavku ćemo vidjeti koji uslovi treba da vrijede za postojanje BDR-a, tj. 
...
\begin{proof}
         ...
         $O^* = \{ x \in \mathbb{R}^n \mid A x \geq b_i, c^T x = v  \}$ je takođe poliedar, $O^* \subset P$. Prethodna teorema nam kaže da $P$ ne sadrži pravac. Iz toga slijedi da ni $O^*$ nema pravac, odakle slijedi da $O^*$ ima barem jednu ekstremnu tačku (stavka 1 prethodne teoreme), označimo je sa $q^*$. Pokažimo da je to i ekstremna tačka od $P$.  Ako bismo pretpostavili da $q^*$ nije ekstremna tačka poliedra $P$, po definiciji slijedi da postoje $y, z \in P$ tako da $q^* = \lambda y + ( 1 - \lambda ) z $, $y \neq q^*, z \neq q^*, \lambda \in [0, 1]$. Kako je $q^* \in O^*$, imamo 
        ...
        odakle imamo $c^T y = v$ i $c^T z = v$, dakle $y, z \in O^*$. Prema tome, $q^*$ nije ekstremna tačka od $O^*$. Prema tome, imamo kontradikciju, odakle slijedi da je $q^*$  ekstremna tačka poliedra  $P$.
\end{proof}
...
% \begin{thm}
 \emph{Posljedica.} Ako  rješavamo problem minimizacije LP-a na poliedru $P$ koji ima barem jednu ekstremnu tačku, tada vrijedi tačno jedna od sljedeće dvije tvrdnje:
   ...
   \end{enumerate}
% \end{thm}
Kako ćemo vidjeti u narednom poglavlju, upravo  ove tvrdnje opravdavaju ideju simpleks metode, koja je bazirana na posjećivanju vrhova poliedra 
...
\end{eqnarray}
Kada se $z$ minimizuje, ova ograničenja osiguravaju da je $z$ veće ili jednako  $\sum_{j \in J} c_{kj}x_j$, za sve $k$. Optimalna vrijednost $z$ neće biti veća od makismuma svih $\sum_{j \in J} c_{kj}x_j$, jer je $z$ minimizovano. Prema tome, optimalna vrijednost varijable $z$ će težiti da bude što manja moguća i jednaka maksimalnoj cijeni nad skupom $K$. Prema tome, model se transformiše u:
\begin{align*}
	&\max z \\
	&\sum_{j \in J} a_{ij} x_j \leq b_i\  \forall i \in I \\ 
	& 	 \sum_{j \in J} a_{kj} x_j \leq z\ \forall k \in K \\
...
\begin{align*}
	...
	& x \geq 0.
\end{align*}
Na kraju, potrebno dozvoliti da je $t \geq 0$, da bi dobili LP model. 
