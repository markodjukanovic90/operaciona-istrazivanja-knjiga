...
odnosno
 ...
 Međutim, pravi razlog za DW dekompoziciju leži u činjenici da za rješavanje problema~\ref{dw3} ne moramo eksplicitno da koristimo sve vrhove politopa $X$. Umjesto toga, krenućemo od jednog   manjeg podskupa (a samim tim i podskupom skupa $\lambda$ promjenljivih). Nakon toga, primijenićemo tehniku ``generisanja kolona'' (engl. column generation), kako bismo u razmatranje uključili nove vrhove politopa $X$, koje će pomoći poboljšanju rješenja Restricted master problema.
 
 Označimo sada problem formulisan  sa~\ref{dw3} Master problem (MP). Na osnovu master problema formiramo tzv. Restricted master problem (RMP)  sa manjim brojem $\lambda$ promjenljivih.
...
Da bismo provjerili da li je optimalno rješenje RMP ujedno i optimalno rješenje master problema, potrebno je da provjerimo da li je reduced cost od svih nebazičnih promjenljivih NIJE JASNO...


i trebamo optimalne vrijednosti dualnih promjenljivih

Označimo vektorom $y$ optimalnu vrijednost dualne promjenljive koja se odnose na ograničenja $\sum_{j\in I} (A x_j)\lambda_j=b$ i skalarom $\alpha$ optimalnu vrijednost dualne promjenljive koja se odnosi na ograničenje $\sum_{j\in I}\lambda_j = 1$. 

Da bi se odredio reduced cost za $\lambda_j$, odredimo šta imamo u odgovarajućoj koloni: U funkciji cilja, imamo $c^Tx_j$, u ograničenjima $Ax_j$ i još jednu jedinicu (na osnovu ograničenja $\sum_{j\in I}\lambda_j = 1$). Tako, kolona na koju se fokusiramo da bismo izračunali reduced cost izgleda ovako
 ...
 i sada rješavamo Column generation subproblem, čiji je cilj da se pronađe najmanji reduced cost. Ukoliko je najmanji reduced cost manji od nule, dobija se nova kolona. U slučaju da je najmanji reduced cost nenegativan, tada je optimalno rješenje RMP ujedmo i optimalno rješenje master problema.
 
 Column generation subproblem se definiše kao
...
Reformulišimo  Column generation subproblem sada kao problem maksimizacije, gdje ćemo za funkciju cilja uzeti negativnu vrijednost funkcije cilja minimizacije
...
$x\in X$ i $Ax=b$. Za to rješenje $x$ važi 	
