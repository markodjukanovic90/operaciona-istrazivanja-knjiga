...
$$\begin{array}{ccccc|c}
    ...
    5 & 0 & -3 & 0 & 0 & -9 \\
\end{array}$$
\fxnote{Marko, ovdje je Milana primijetila da u prvoj koloni ima broj 7 koji je pozitivan. Kako onda nema elemenata koji su strogo veci od nule (njena primjedba oznacena crvenom bojom) Nisam dalje citao ovaj primjer, na ovo se moramo zajednicki vratiti. Marko: pogledati sada je li ok...} 
...
\begin{primjer}
\emph{Cjelobrojni problem ruksaka.} Neka je dato $n$ proizvoda i neka je težina $i$-tog proizvoda jednaka $w_i$, dok je vrijednost $i$-tog proizvoda jednaka $c_i$, $i=1,\ldots,n$. Koje proizvode treba da stavimo u ruksak  čiji je kapacitet $C>0$ tako da   vrijednost proizvoda u ruksaku bude maksimalna? 

\emph{Rješenje}. Definišimo promjenljive odluke $x_i$, koje dobijaju vrijednost 1 ako  $i$-ti proizvod uključujemo u ruksak, odnosno, vrijednost 0, ako $i$-ti proizvod nije uključen u ruksak, za $i=1,\ldots,n$. Ograničenje kapaciteta ruksaka se modeluje ograničenjem $\sum_{i=1}^n w_i x_i \leq C$. Funkcija cilja maksimizuje ukupnu vrijednost odabranih proizvoda i modeluje se uz pomoć linearne kombinacije $f(x) = \sum_{i=1}^n c_i x_i$. 
\end{primjer}
\begin{primjer}
...
\emph{Pronaći minimalan broj medicinskih sestara koje bolnica treba da zaposli, da bi se mogle napraviti takve smjene.}
...
\end{table}
\fxnote{Dragan: stavio sam da je broj gledalaca televizije 10000 - pisalo je 1000}
...
gdje je
\begin{itemize}
	\item $x_i$: binarna promjenljiva, koja ozačava da li je (1) ili nije (0) otvoreno postrojenje $i$;
...
$$  \sum_{j=1}^n x_{ij} = 1, \forall i =1,\ldots, n$$ 
 ...
 \fixme{Stavio sam ovo u podnaslove. Ako mislis da ne treba, vrati, jer nisam ni ja siguran da li je bolje ovako ili onako kako je prvobitno bilo}
...
(2) pokrenuti reklamnu kampanju ili (3) razviti novi proizvod. Jasno je da su to logičke odluke, odnosno odluke tipa ``da ili ne'', pa ih, prema tome, modelujemo   uvođenjem  binarnih promjenljivih $x_i \in \{0, 1 \}$, $i=1,2,3$. Često se nameće uslov da se najviše jedna od ovih odluka izvrši, što se modeluje izrazom $\sum_{i=1}^3 x_i \leq 1$. Ovo ograničenje se obično naziva \emph{ograničenjem sa višestrukim izborom} (eng. \emph{multiple choice constraint}), jer se njime ograničava izbor odluka na najviše jednu od, u ovom slučaju, tri dostupne alternative. Ukoliko se zahtijeva da se tačno jedna od ovih odluka izvrši, onda se znak nejednakosti u posljednjem izrazu mijenja znakom jednakosti, odnosno, ograničenje postaje $\sum_{i=1}^3 x_i = 1$.


\fxnote{marko, ovaj dio sa proizvodnjom lijekova nije bas jasan. Da li hoces da se umjesto y promjenljive koristi Kx ili obrnuto? Mozes ti ukljuciti jednu recenicu prije donjeg teksta, da se objasni motivacija sta se tu zapravo radi}
 
Pretpostavimo da proizvođač lijekova treba da odluči hoće li koristiti rezervoar za fermentaciju u proizvodnji. Ako koristi rezervoar, tehnologija obrade zahtijeva da se napravi $K$ jedinica ljekova. Dakle, količina proizvodnje $y$ dobija vrijednosti 0 ili $K$, a problem se može modelovati binarnom varijablom $x_j \in \{ 0, 1\}$, a potom zamjenom $Kx_j$ za $y_i$. 
...
\fixme{Marko, ovi uslovi za y promjenljive mi nisu u redu. Prvo, moze li neka od y promjenljivih biti 0, posto pise da pripadaju skupu N? Ako nijedno y ne moze da bude nula, onda $y_1 + y_2 + y_3 \leq 2$ nikad nije zadovoljeno. E sad, ako su dvije y promjenljive nule, onda tacka x mora da zadovoljava uslove za dva regiona, sto je nemoguce. Ako ja vidim dobro, uslovi bi trebali da izgledaju ovako kako sam ispod napisao , odnosno da su tacno dvije y promjenljive jednake 1, a treca jednaka nuli. }
\begin{align*}
    & y_1 + y_2 + y_3 = 2 \\
& y_1, y_2, y_3 \in \{0,1\}.
\end{align*}
...
$$f(x) = \begin{cases}
      ...
      \end{cases}
$$
gdje su $I_1, I_2$ i $I_3$  zatvoreni intervali iz $\mathbb{R}$, koji se nastavljaju jedan na drugi.
...
% \emph{Eliminacija proizvoda varijabli}.
 ...
 Ova tehnika može generalizovati na slučaj proizvoda $n$ binarnih promjenljivih. Definišimo promjenljivu  $y = x_1 \cdot \ldots \cdot x_n$, $x_i\in\{0,1\}, i=1,\ldots ,n$. Uključivanjem sljedećih ograničenja 
   \begin{align*}
   	&y \leq x_1 \\
   	& \vdots \\
   	&y \leq x_n \\
...
\fixme{marko, ovaj donji paragraf mi je nejasan. U kojoj prethodnoj stavci je pomenuta aproksimacija pomocu niza po dijelovima linearnih funkcija?}
...
\begin{primjer} Pretpostavimo da je stanovništvo koncentrisano u $I$ okruga  unutar grada i da u okrugu $i\in [I]$ stanuje $p_i$ ljudi. Preliminarna analiza (premjeri zemljišta, političke i društvene prilike itd.) ukazala je da potencijalne lokacije za izgradnju vatrogasnih domova mogu biti na $J$ lokacija. Neka $d_{ij} \geq 0$ predstavlja udaljenost   središta okruga (dijela grada) $i$  do lokacije $j$. 
\end{primjer}
...
\end{equation}
Pretpostavimo da je jedan okrug posebno osjetljiv na požar i da lokacije  1 i 2 ili lokacije 3 i 4 treba  da budu iskorištene da zaštite taj okrug. Ovakva situacija se modeluje sa
\begin{equation*} 
...
\end{equation}
 ...
 \section{Kompleksnost rješavanja cjelobrojnog programiranja}\label{sec:kompleksnostILP}
   ...
   \begin{align*}
    ...
    \end{align*}    
\fixme{marko, da li ima potrebe da se objasnjava sta su ove x promjenljive?}
   ...
   \begin{proof}
   	   ...
   	   $x_1,\ldots, x_n$.  Konstruišimo ILP instancu sa varijablama $z_1, \ldots, z_n$ na sljedeći način.
   	   Svaka klauzula $\phi_i(x)$ je konvertovana u nejednakost,  gdje važi:
   	   \begin{itemize}
   	   	\item $z_i = x_i$, ako je $x_i$ pozitivan literal u klauzuli $\phi$, odnosno 
   	   	\item $z_i = 1 - x_i$ ako je $x_i$ negativan literal u klauzuli $\phi$ (negacija ispred).
   	   \end{itemize}
   	   ...
   	   Dakle, iz gornje redukcije, pronalazak rješenja ILP instance odgovara određivanju dodjele $\overline{x}$ literalima, tako da je formula $\phi$ zadovoljena.\fixme{marko, ovu recenicu popravi: Dakle, treba da se provjeri da li je ILP instanca dopustiva.} Takođe, lako se vidi da je ova redukcija polinomna. Prema tome, ILP je NP-težak, s obzirom da je 3--SAT problem NP-kompletan. 
   \end{proof} 
 Može se pokazati i sljedeća teorema.
 \begin{thm} Neka je $k\in \mathbb{Z}$.
 	 Problem odlučivanja da li postoji cjelobrojno rješenje  ILP-a,  vrijednosti manje od $k$ je NP--kompletan, uz pretpostavku da je posmatrani ILP problem minimizacije. 
 \end{thm}
...
%	\end{table}
	...
	Sa $D_c$ označimo maksimalnu daljinu koja se može pokriti instalacijom $c$. Pod instalacijom možemo da podrazumijevamo skladiše, antene i sl. Pod ``pokrivanjem'' podrazumijevamo ispunjavanje zahtjeva klijenata, koji žive u nekom mjestu. Sa $c_i$ označimo cijenu instalacije  na mjesto $i$. Sa $d_{ij}$ označimo udaljenost između mjesta $i$ i $j$. Zadatak ovog problema je da se odredi koja mjesta odabrati za lociranje instalacija, tako da su sva mjesta pokrivena, ali da pri tome minimizujemo troškove instalacija. Sa jednog odabranog mjesta se pokrivaju sva susjedna mjesta koja su u njegovom radijusu. 
...
%http://courses.ieor.berkeley.edu/ieor151/lecture_notes/ieor151_lec14.pdf
	\item Potrebno je modelovati \emph{$P$-center problem čvorova} (eng. \emph{vertex p-center problem}). Formulacija problema je sljedeća. Ulazni podaci su predstavljeni neusmjerenim grafom $G = (I, J, E)$, gdje su objekti potražnje predstavljeni  vrhovima $i\in I$, a moguće lokacije (servisnih) objekata date su drugim skupom vrhova $j \in J$, Grane $e_{i,j} \in E$ postoje samo između čvorova $i \in  I$ i čvorova $j \in J$. Dalje, parovima čvorova su dodijeljene pozitivne težine $d_{i,j}\geq 0$, što predstavlja udaljenost između čvorova $i$ i $j$. Treba imati na umu da je moguće imati i udaljenost 0 između čvora potražnje i moguće lokacije objekta. Za svaki čvor $i \in I$ pridružena je težina zahtjeva $h_i>0$, koja predstavlja količinu zahtjeva koje taj čvor može da ispuni. 
	
	Zadatak je postaviti $p$ objekata kako be se smanjila maksimalnu udaljenost između bilo kojeg čvora potražnje i	njegovog servisnog objekta. 
...
\begin{itemize}
	...
	\item Ako se neko ulje uključeno u upotrebu, onda se mora iskoristiti barem 30 $t$ tog ulja.
