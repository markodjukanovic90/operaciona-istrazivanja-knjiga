...
\section{Problemi Modelovani preko Linearnog Programiranja}\label{sec:modeli_lp}
...
\end{definition}
\begin{definition}
   Neka je $a\in \mathbb{R}^n $ i $b \in \mathbb{R}$. Skup $\{ x \in \mathbb{R}^n \mid a^T x = b \}$ se naziva hiperravan u $\mathbb{R}^n$ čiji je vektorom normale $a$, dok se $\{ x \in \mathbb{R}^n \mid a^T x \geq b \}$ naziva poluprostor. 
\end{definition}
Pokažimo sada da je poliedar konveksan skup. To je bitna karakteristika, jer, kako znamo, uslovi  problema LP-a se mogu zapisati kao poliedar, a ako je on konveksan skup, prethodna teorema nam kaže da se onda može naći globalni minimum (maksimum), tj. optimalno rješenje zadanog problema LP-a (ako postoji lokalni minimum).
\begin{prop}
...
\begin{definition}
      ...
      $I = \{ i \in M_1 \cup M_2 \cup M_3 \mid a_i^T x^* = b_i \}$  skup indeksa aktivnih u $x^*\in \mathbb{R}^n$.  Ako je skup $\{ a_i \mid i \in I \}$ linearno nezavisan onda se $x^*$  naziva bazno rješenje. 
\end{definition}
...
\end{definition}
Pokažimo sada da su vrhovi, ekstremen tačke poiedra i bazna dopustiva rješenja jedno te isto kada je riječ o LP-u.
\begin{thm}
...
\section{Neki problemi modelovani pomoću ILP-a}
...
$$  \sum_{j=1}^n x_{ij} = 1, \forall i =1,\ldots, n$$ 
 ...
 \section{Neke tehnike konstrukcije modela Cjelobrojnog Programiranja}
...
\end{equation}
 ...
 \chapter{Tehnike rješavanja problema Cjelobrojnog programiranja}
...
$$ \begin{array}{lll}
 ...
 \section{Benderova Dekompozicija}
  ...
  \section{Lagranžova Dekompozicija}
...
Ovaj problem je NP--težak. Ideja je riješiti ga uz pomoć Problema pokrivanja skupa i 
njegovih algoritama. Pokazaćemo da postoji $2\cdot H_n$--aproksimacija za problem najkraćeg nadstringa. 
...
\section{Heurističke Metode}
 ...
 \begin{itemize}
 	...
 	\item Lagranžova relaksacija (eng. Lagrangian relaxation - LR);
 	\item neuronske mreže (eng. Neural network algorithm - NNA);
...
Upisivanje modela u .lp format se vrši pozivom funkcije \emph{writeLP()}, čiji je argument naziv fajla. 
 ...
 \end{verbatim}
 \section{Primjena rješavača PuLP na rješavanje Problema Lokacija}%Capacitated Facility Location
 ...
 U ovoj sekciji modelujemo problem najdužeg podniza bez ponavljanja. Ovaj problem kao ulaz prihvata dva (proizvoljna) stringa $s_1$ i $s_2$ nad nekom abecedom, a kao izlaz vraća string $s$ maksimalne dužine sa sljedećim karakteristikama:  
 \begin{itemize}
...
Za primjer, neka su data dva stringa $s_1 = \texttt{abdcccab}$ i $s_2=\texttt{abccacd}$, rješenje ovog problema je $s=\texttt{abc}$. Primijetimo da je $s$ zajednički podniz za oba ulazna stringa te da ispunjava uslov o ``neponavljajućim'' karakterima. Takođe, može se provjeriti da ne postoji duži zajednički podniz sa takvim svojstvom za ove ulazne stringove, što znači da je rješenje optimalno. Konstruišimo sada jedan Binarni cjelobrojni model za ovaj problem. Nazovimo par $(a, b), 1 \leq i \leq |s_1|, 1 \leq j \leq |s_2|$ \emph{mečing} akko je $s_1[a]=s_2[b], a < b$. Dalje, sa $\mathcal{M}$ označimo skup svih mečinga za dati ulaz $S=\{s_1, s_2\}$. Takođe, označimo sa  $\mathcal{M}_a$, skup svih mečinga koji pokrivaju isto slovo, tj. 
$\mathcal{M}_{\texttt{a}} = \{ (i, j) \mid s_1[i]=s_2[j]=\texttt{a}\}$, $\texttt{a} \in \Sigma$. Za svaki mečing $p=(p(a),p(b))$, pridružimo jednu binarnu varijablu $z_{p} \in \{0, 1\}$. Označimo $l(z_p)= s_1[ p(a) ], Z_{\texttt{a}} = \{ z \mid l(z) = \texttt{a}\}, \texttt{a}\in \Sigma$, te skup svih vaarijabli sa $Z = \cup_{\texttt{a} \in \Sigma} Z_{\texttt{a}}$. 
...
\begin{equation}\label{eq:rflcs-max-1}
	 \sum_{z \in  Z_a } z \leq 1, \forall a \in \Sigma 
\end{equation}
Takođe, s obzirom da rješenje problema odgovara nalasku skupa (međusobno) nekonfliktnih mečinga (odgovarajućih $z$-varijabli) maksimalne kardinalnosti, dodajemo model
