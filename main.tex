...
\end{poslje}
 ...
 Nadalje, formalno definišimo ekstremne tačke, vrhove i bazna dopustiva rješenja koja igraju bitnu ulogu u teoriji linearnog programiranja. 
...
\end{align}
 ...
 Može se pokazati da za optimalno rješenje vrijedi  $x^{+}\cdot x^{-}= 0$.  
   Pretpostavimo da je $x_j^*$, $j \in J$ optimalno rješenje LP-a (\ref{eq:lp_abs}).  Lako je vidjeti da je ekvivalentno rješenje LP-a (\ref{eq:lp_abs_transformed})  dato u obliku 
  $(x_j^+)^* = x_j^*$  ako je $x_j^* \geq 0$, a inače $(x_j^+)^* = 0$ takođe dopustivo, ali i optimalno za model (\ref{eq:lp_abs_transformed}). Slično imamo $(x_j^-)^* = - x_j^*$  ako je $x_j^* \leq 0$, a inače $(x_j^-)^* = 0$. Lako se provjeri da su vrijednosti ova dva rješenja u odnosu na odgovarajuće funkcije cilja jednake. Ako bi  optimalna tačka LP-a bila neka tačka $(x^+, x^-)$,  za koju postoji neko $j$ tako da  $x^{+}_j, x^{-}_j \geq 0$, onda bi zamijena jedne od optimalnih vrijednosti $x^+_j$ ili $x^-_j$  sa $x^{+}_j - x^{-}_j$
  dodatno doprinijelo smanjenu funkcije cilja za $|x^{+}_j - x^{-}_j|>0$, što je kontradikcija. Dakle,   optimalne tačke LP-a (\ref{eq:lp_abs_transformed}) su jedino one za koje je barem jedna od promjenljivih $x_j^+$ ili $x_j^-$, $j \in J$  jednaka 0, pod uslovom da LP (\ref{eq:lp_abs}) ima barem jedno dopustivo rješenje. Zaključujemo da je ova transformacija validna. Primijetimo  još da iz  nedopustivosti jednog modela slijedi nedopustivost drugog modela i obrnuto. %u smislu jednake optimalne vrijednosti. 
