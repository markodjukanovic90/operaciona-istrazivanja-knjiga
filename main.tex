...
Ovaj algoritam je takodje 2--aproksimacioni. Interesantno je napomenuti da za Problem binarnog pakovanja ne postoji aproksimacioni algoritam čiji je faktor $c <\frac{3}{2}$ (osim ako je $P=NP$), kako je pokazano u literaturi. 

 \subsubsection{Problem Pokrivanja Skupa: aproksimativni algoritmi}
Ovaj problem je uopstenje Problema pokrivanja čvorova, i definisan je na sljedeci naćin.
\begin{definition}
...
\begin{proof}
	  ...
	  Cijenu selektovanja $f_i$ distribuiramo podjednakom vjerovatnocom na sve elemente koji su po prvi put pokriveni sa $f_i$. Posmatrajmo primjer dat na Slici~\ref{fig:set-cover-1}. U prvoj iteraciji pohlepnog algoritma,  
	 ...
	 Neka $ c_x$ oznacava cijenu za element $x\in X$.  Svakom elementu se cijena dodijeli najviše jednom, onog puta kada se pokrije po prvi put. Ako se $x$ pokriva prvi put sa $f_i$, tada imamo:
	 ...
	 \end{align}
	  Pokazimo sljedeću kljucnu nejednakost koja služi u dokazu ove teoreme. Za svaku $f \in \mathcal{F}$, vrijedi  
	 ...
	 Ostaje nam jos da pokazemo formulu~(\ref{eq:f2x}). 
	 
	 Neka je $f\in \mathcal{F}$ tako da za $i=1,\ldots,|C|$
	 $$u_i(f) = |f \setminus (f_1 \cup \cdots \cup f_{i} )|$$
	 
	 označava ostale elemente u $f \in \mathcal{F}$ koji su jos uvijek nepokriveni nakon sto je algoritam odabarao skupove $f_1,\ldots, f_i$ u pokrivaču $C$ sve do $i$-te iteracije. Definišimo $u_0(f)=|f|$ ko broj elemenata skupa $f$ koji je inicijalno nepokriven. 
	 Označimo sa $k$ prvi indeks tako da je $u_k(f)=0$  -- što znači da su svi elementi skupa $f$ u $k$-toj iteraciji
	 pokriveni prvi put nekim skupovima $S_1,\ldots,S_i$ koji su odabranu u ranijim koracima algoritma. Primijetimo da vrijedi
	 ...
	 po prvi put sa $f_i$ u $i$-toj iteraciji za $i=1,\ldots,k$. Prema tome, vrijedi
 ...
 \begin{align}\label{eq:q2x}
 	|f_i \setminus (f_1 \cup \cdots \cup f_{i-1})| \geq | f   \setminus (f_1 \cup \cdots \cup f_{i-1}) | = u_{i-1}(f),
 \end{align}
 jer je $S_i$ gridi izbor u $i$-toj iteraciji i svaki drugi izbor ne može pokriti više novih (nepokrivenih elemenata) od ovog skupa.
