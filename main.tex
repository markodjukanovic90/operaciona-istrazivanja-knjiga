...
\begin{array}{lllll}
 ...
 \end{array}
$$
Označimo funkciju cilja sa $f$. Iz uslova je jasno da su gornje granice date sa $f(x_1,x_2) = x_1 + x_2 \leq 4 x_1 + 3 x_2 \leq 9$, kao i 
...
Uradimo sada slično izvođenje u opštijem obliku. Iskoristimo ograničenja data u modelu, i iskombinujmo ih sa koeficijentima funkcije cilja na sljedeći način. Neka su $y_1, y_2 \geq 0$ koeficijenti sa kojima množimo ograničenja u početnom modelu. Tada imamo 
\begin{align*}
...
\begin{align}
	...
	& x \geq 0.
\end{align}
odgovorarajući dualni problem, označen sa ($D$), je dat sa 
\begin{align}
     & \min b^T y  \\
     & \mbox{t.d.} \nonumber \\
     & A^T y \geq c \\
...
\begin{poslje}
	Ako je $y$ dopustivo rješenje problema  ($D$),  onda bilo koje dopustivo rješenje $x$ problema ($P$) je ograničeno odozgo sa $b^T y$. Prema tome, ako je problem ($D$) dopustiv, onda je ($P$) ograničen. Slično se može pokazati da ovo vrijedi i kada problemi ($P$) i ($D$) zamijene uloge.
\end{poslje}
...
\begin{proof}
         ...
         Primijenimo simpleks metod, pa posmatrajmo posljednju  simpleks tabelu, odnosno tabelu iz koje je dobijeno rješenje $x^*$.
...
Dalje, važi i $c^T x = ((c^*)^T + (y^*)^T A ) x$, odnosno     $ c = A^T y^* + c^*$. 
    ...
    Kako je $c^* \leq 0$, imamo  $A^T y^* \geq c$. Kako je $-y^* \leq 0$, slijedi da je $y^*$ dopustivo rješenje za  ($D$).  Iz (\ref{eq:f_y_star}) je pokazano da je vrijednost funkcije cilja u $y^*$ upravo $f^*$. Iz Teoreme slabe dualnosti slijedi da je $y^*$ optimalno rješenje problema  ($D$). 
    
     \fxnote{Marko, je li ovo slijedi iz teoreme, ili iz propozicije?}
...
\fxnote{Marko, slazem se sa Milanom da primjer uradimo na ovom mjestu.}
...
\fxnote{dragan: prebacio sam zadatak medju zadatke}
...
\fixme{Dragan: dodao sam zadatak na kraju poglavlja iz simpleks metode}
...
Postoji nekoliko metoda ove vrste, kao što su: metod centralne putanje, redukcija potencijala, afino skaliranje, itd. U ovoj sekciji ćemo razmatrati metod centralne putanje. Napomenimo i da ćemo se baviti rješavanjem problema LP-a koji je dat u obliku (\ref{eq:lp_equality_constraint}). 
...
\fixme{dovde sam stao - prebacujem se na Fakrasovu lemu}
\begin{definition}
...
\begin{proof}
 ...
 Ako pogledamo desnu stranu ograničenja, vidimo da je vektor $x = 0$ 
  ...
  \fxnote{umjesto optimalan primal, u tekstu sam prepravio na: primal ima optimalno rjesenje}
...
\end{figure}
Iz geometrijske interpretacije slijedi da se vektor $c =(1,4)^T$ može predstaviti kao linearna kombinacija vekotora vrsta matrice $A$, kao što vidimo na Slici~\ref{fig:farkas-slika}. Vektor $c =(1,4)^T$ se nalazi unutar prostora upregnutog   vektorima $a_1, \ldots, a_n$. što implicira da početni sistem nema rješenje. 
Kada bismo posmatrali $c=(5,0)^T$, zaključili bi da se on ne može izraziti preko vektora vrste matrice $A$, jer je van prostora upregnutog sa vektrima vrste. Prema tome, u slučaju ove konfiguracije bi postojalo  rješenje za početni problem.  
...
\emph{Da li je optimalna vrijednost funkcije cilja problema LP-a veća od neke unaprijed zadate  vrijednosti k?}
 Na ovo pitanje možemo odgovoriti pomoću  \emph{Fourier–Motzkin}-ove eliminacije. Ova metoda je strukturalno drugačija  od simpleks metode. Ideja algoritma je zasnovana na redukciji problema sa $n$ promjenljivih na ekvivalentan problem sa $n-1$ promjenljivih. Ekvivalentnost podrazumijeva da početni problem ima rješenje akko  ga ima i redukovani problem. Eliminacija promjenljivih se vrši iterativno, odnosno, u svakom koraku se smanjuje broj promjenljivih. Na kraju se početni sistem svodi na sistem sa jednom promjenljivom, za koji se lako može provjeriti da li sadrži dopustivo rješenje.
 
 Ponovimo, glavni korak ovog metoda je redukcija problema na ekvivalentan problem sa manjim brojem promjenljivih. Pretpostavimo da su ograničenja data u varijablama $x_1,\ldots, x_n$ i da želimo da eliminišemo varijablu $x_n$. Za svako od ograničenja
...
\fxnote{obratiti paznju na znak prve nejednacine, ne poklapa se sa racunom dole}
\begin{align*}
	 &x - y \geq 1 \\
	 &-x + 2y \geq 1 \\
	 & 3x - 5y \geq 1 \\
	 & x,y \geq 0.
\end{align*}
Eliminišimo promjenljivu $y$ iz sistema. Kako je $x-y \leq 1 \Rightarrow y \leq x -1 $, 
...
\item Koristeći Farkaš-ovu lemu, pokazati sljedeći rezultat. 
 ...
 Koristeći Fourijer-Motzkin-ovu eliminaciju, pokazati da postoji rješenje za sljedeći sistem nejednačina:
 \begin{align*} 
 	&x - 5y + 2 z \geq 7 \\
 	&3x - 2y - 6 z \geq -12 \\
 	& -2x + 5y - 4z \geq -10 \\
 	...
 	& x,y,z \geq 0.
 \end{align*}
\item Pomoću Fourijer-Motzkin-ovu eliminacije, pokazati sa je sistem 
\begin{align*}
	&x_1 \geq 0 \\
	&x_2 \geq 0 \\
	& x_1 + x_2 \leq -2 \\
