...
Sljedeći primjer demonstrira odgovarajuće geometrijske korake (``šetanje'') po dopustivom regionu u odnosu na iteracije simpleks metode koji se izvršavaju.   %pratiti https://faculty.math.illinois.edu/~mlavrov/docs/482-fall-2019/lecture5.pdf (strana 2 i 3) 
 Neka je dat problem:
 \begin{align*}
 	&\max 2x + 3y \\
 	& -x + y \leq 3 \\
...
Simpleks metod ne garantuje   rješavanje LP-a u polinomijalnom broju koraka u odnosu na veličinu ulaza. % V. Klee and G. Minty constructed an example LP: simplex method needs 2^n iterations to solve it 
...
\end{align} 
što nas (uz uslov komplementarnosti) dovodi do:
\begin{align}
	&Ax = b \\
	&A^T y + s = c \\
...
Prema tome, dobili smo sistem od $n-1$ varijabli koji se sastoji od  $k \cdot l$ nejednakosti gore opisanih te originalnih ograničenja koja ne uključuju $x_n$ u sebi. Ovaj proces nastavimo iterativno dok ne dobijemo samo jednu promjenjivu, te potom trivijalno riješimo sistem. 
...
Nije realno očekivati da varijable odluke dobijaju neprekidne vrijednosti u svim problemima. Recimo, ako se zahtjeva u problemu rasporeda ljudskih resursa no smjenama koje se ti?u minimalnog broja osoblja u smjenu. Ovaj broj je cjelobrojan, preciznije prirodan broj. U slučaju da su sve varijable odluke u modelu problema cjelobrojne, riječ je o problemu cjelobrojnog programiranja (eng. \emph{Integer programming}). 
 ...
 \begin{itemize}
   ...
   \item $x_1$ je binarna, $x_2$ je realna (pozitivna) varijabla, $0 \leq x_2 \leq u$. opet uvodimo istu smjenu $y= x_1 x_2$ koja je neprekidna, te dodamo sljedeća ograničenja da primoramo $y$ da primi vrijednost proizvoda:
    \begin{align*}
    	 ...
    	 & y \geq 0.
    \end{align*}
  \item $x_1, x_2$ u obje neprekidne varijable, $0 \leq x_1 \leq u_1$, $0 \leq x_2 \leq u_2$. Uvedemo varijable $y_1 = \frac{1}{2}( x_1 + x_2) $ i  $y_2 =\frac{1}{2}(x_1 - x_2)$. Tada se izraz $x_1x_2$ može predstaviti u separabilnoj formi $y_1^2 - y_2^2$ koja se može aproksimirati po-dijelovima-linearnim funkcijama. Granice za vrijednosti novih varijabli su 
...
\end{equation}
 ...
 Posmatrajmo problem linearnog programiranja
   ...
   \begin{equation}
 	\begin{aligned}\label{primjer:dw1}
 		...
 		&x \in X=\{(x_1,_2):0\leqslant x_1,_2 <\leqslant 1\}.
 	\end{aligned}
 \end{equation}
 ...
 Prije nego što na ovom problemu ilustrujemo upotrebu DW dekompozicije, riješimo problem na neki od (u ovom slučaju) bržih načina. Ako problem predstavimo grafički (Slika XXX), vidimo da je dopustivi skup osjenčeni trougao, a da se maksimalna vrijednost funkcije cilja dostiže u vrhu $(\frac34, 
...
\end{aligned}$$
 Preformulacijom problema~\ref{primjer:dw} dobijamo
 	...
 	\begin{aligned}\label{primjer:dw3}
 		\min\  & \sum_{j=1}^4(c^T x_j)\lambda_j\\
...
Izabraćemo promjenljive $\lambda_1, s_1$ i $s_2$ kao inicijalne osnovne promjenljive. Pošto  promjenljive $\lambda_2,\lambda_3$ i $\lambda_4$ ne egzistiraju u potproblemu,  RMP izgleda ovako
	...
	\end{aligned}
\end{equation}
Posljednji problem se trivijalno rješava (funkcija cilja ima vrijednost 0, a vrijednost promjenljivih se direktno očitavaju iz ograničenja). Ispitajmo sada šta šta je optilalna vrijednost dualnih promjenjivih, dakle $y$ i $\alpha$ uz pomoć simpleks tabele problema \ref{primjer:dw6}. Ona izgleda ovako
\begin{center}
...
\end{tabular}
\end{center}
kako je ovo optimalna simpleks tabela, 
Posmatrajmo sada dualni problem problema~\ref{primjer:dw3}. Označimo sa $y_1$, $y_2$ i $\alpha$ promjenljive dualnog problema. Kako je ova tabela i optimalna simpleks tabela, vrijednost optimuma dualnih promjenjivih jednaka je odgovarajućim doprinosima (posljednji red tabele), pa je $y_1 = 0 = y_2 = \alpha$. 

%Za dalje razmatranje biće nam potrebna  Tabela~\ref{tbl:dw1}, koju ćemo popunjavati u iterativnom procesu.
\begin{table}\centering
	\begin{tabular}{|c|c|c|}
		\hline
		$(y_1,y_2,\alpha)=c^T_B B^{-1}$ & $\overline{z}= c^T_B B^{-1}b$&$BV$  \\
		\hline
		$B^{-1}$& $B^{-1}b$ &$\vdots$  \\
		\hline
...
\end{align} 
Optimalna vrijednost problema (\ref{eq:cg-1}) je jednaka $\hat{z}=1$, i dostiže se, recimo, u ekstremnoj tački $x^2= (1, 0)^T.$ Kako je optimalna vrijednost pozitivna, ulazna kolona u prethodni potproblem postoji koju treba da odaberemo. Ova kolona (ako posmatramo problem (\ref{primjer:dw5})) će biti data sa 
\begin{align}
     \begin{pmatrix}
     	    c^T x^2 \\
     	    A x^2  \\
...
Kada pogledamo prethodne vrijednosti kolone i master problem (\ref{primjer:dw5}), jasno je da ona odgovara koeficijentima varijable $\lambda_2$. Dakle, prethodna kolona je data u odnosu na $\lambda_2$, ali je sada prebacimo na trenutnu bazu datu sa $s_1, s_2$ i $\lambda_1$, gdje je $B^{-1} =  \begin{pmatrix}
	...
	0 & 1 & 0  \\
	0 & 0 & 1  \\
\end{pmatrix}$ da bismo dodali potrebnu kolonu za RMP. Za to je potrebno izvršiti transformacija na nove baze opisane u Tabeli \ref{tbl:dw1}, odakle dobijamo kolonu za dodavanje u RMP:
...
Ubacimo za trenutak ovu kolonu u prethodnu simpleks tabelu, odakle imamo tabelu: 
 ...
 \begin{center}
 	...
 	\end{tabular}
 \end{center}
Primijetimo da je cijena doprinosa nove kolone promijenjena sa 1 na -1 (zbog korištenja tabelarne forme simpleks metoda, koeficijenti doprinosa se množe sa -1). Sada koristimo novu kolonu kao ulaznu kolonu za transformacije -- u njoj tražimo kandidata za pivota kako smo to navikli u simpleks metodi. Odaberemo element čija je vrijednost $2$, pa pivotirajmo oko njega. Prvo sve elemente u koloni 2 podijelimo sa 2, te potom izvršimo elementarne transformacije u kojima načinimo nule u dodatoj koloni. Varijabla $s_2$ izlazi iz baze, dok $\lambda_2$ ulazi. Na kraju dobijamo transformisanu tabelu:
...
\begin{pmatrix}
	   ...
	   0 & -\frac{1}{2}    &  1  \\
\end{pmatrix}$
na osnovu prethodne simpleks tabele, dobijamo optimalne vrijednosti dualnih varijabli iz koeficijenta doprinosa, tj. $y_1 = 0, y_2 = \frac{1}{2}, \alpha = 0$. \\
...
\end{align}
Rješenje ovog problema je lako dobiti, tj. optimum se dostiže u, recimo,  tački $x^3 = (0, 1)^T$, dok je optimum jednak $\hat{z}= \frac{3}{2}$. Kako je $\hat{z} \geq \frac{3}{2}$, treba da odaberemo novu kolonu koja se dodaje u RMP. 
...
Upisivanje modela u .lp format se vrši pozivom funkcije \emph{writeLP()}, čiji je argument naziv fajla. 
 ...
 \section{Primjena Cplex-a na jedan String Problem}
