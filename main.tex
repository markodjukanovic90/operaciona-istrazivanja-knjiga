...
Nakon razgovora sa rukovodstvom firme, utvđeni su glavni ciljevi istraživanja. Naime, došlo se do sljedeće definicije (idealizovanog) problema koji treba da se riješi:
\begin{itemize}
    ...
    \item Dopuštena je bilo koja kombinacija stope proizvodnje koja zadovoljava   ograničenja, uključujući i slučaj da se ne proizvede niti jednan od (dva) proizvoda kao i slučaj da se proizvede maksimalan broj drugog proizvoda. 
\end{itemize}

Identifikovane su informacije koje treba prikupiti prije modelovanja:
\begin{itemize}
...
Budući da značajniji troškovi za pokretanje proizvodnje i promovisanje novih proizvoda ne postoje, ukupna dobit je aproksimirana dobiti po proizvedenoj seriji koja je pomnožena sa brojem serija proizvodnje.  

Nakon ovoga, trebalo je procijeniti konstante (parametre)  u modelu. Ovaj proces je tekao u komunikaciji između uprave firme i tima zadužen za rješenje problema. 
...
\section{Konstrukcija simpleks metode}
 ...
 Neka je $x = [x_1,\ldots, x_n]^T$ bazno dopustivo rješenje poliedra $P$, gdje je $B$ matrica baze, $x_B$ bazni dio vektora $x$, a gdje su na mjesto nebaznih indeksa $N$ postavljene 0.  Ideja simpleks metode je  ``šetanje'' po ivici poliedra  do drugog (baznog) dopustivog rješenja dok ne nađemo optimalno rješenje. Krenimo pravcem $d$ u  početnoj tački $x$  pri čemu trebamo obratiti  pažnju na to da je zadovoljen
\begin{itemize}
...
\end{itemize}
Birajmo nebaznu koordinatu u $x$, neka je to $x_j, j \in N$. Definišemo $d=(d_B,  d_N)$, gdje su koordinate od (nebaznog dijela) $d_N$ svugdje 0, osim na na koordinati $j$ za koju prima vrijednost 1.   Bazni dio vektora $d$, koji je označen sa $d_B$, izvešćemo iz uslova dopustivosti i pretpostavke da ``šetamo'' po granici dopustivog regiona poliedra $P$: $Ax + \lambda Ad = b$, odakle je $Ad = 0$. Tada vrijedi i
$$ Ad = \sum_{i=1}^n A_i d_i = \sum_{i=1}^m A_{p_i} d_i + A_j = B d_B + A_j = 0$$ za one koordinate $p_i$ koje pripadaju baznom dijelu, 
odakle je $d_B = -  B^{-1} \cdot A_j $.  Za ovako odabran $d$ imamo zadovoljen uslov dopustivosti jer $A( x + \lambda d )  =b$. Posmatrajmo sada uslov nenegativnosti, tj. $x + \lambda d \geq 0$:
\begin{itemize}
...
Ostalo nam je još odrediti dužinu koraka pravca $d$, tj. vrijednost $\lambda>0$. Ideja je da se krene nekim smjerom dok god se ne dođe do drugog vrha, tj. treba odrediti dužinu vektora $d$ koji je jednak $\max \{ \lambda \in \mathbb{R} \mid x + \lambda d \geq 0 \}$. Razlikujemo sljedeća dva slučaja:
\begin{itemize}
...
U osnovi, simpleks algoritam se sastoji od stalne izmjene jedne kolone baznih vektora u $B$ sa nekim nebaznim vektorom (i odgovarajućom kolonom) iterativno u cilju smanjenja trenutne vrijednosti cilje funkcije, dok god je to moguće. 
...
Na osnovu ovakve tabele, te koraka simpleks metode opisanih u prethodnoj sekciji, sljedeći koracu se izvršavaju u tabelarnom obliku simpleks metode:
\begin{itemize}
    \item Konstruisati inicijalnu simpleks tabelu sa početnom bazom $B$ i odgovarajuće bazno dopustivo rješenje $x$.
    \item Posmatrati koeficijente doprinosa $\overline{c_j}$ (posljednja vrsta tabele). Ako su svi $\overline{c_j} \geq 0$, rješenje se ne može dalje popraviti, pa smo našli optimalno rješenje. U protivnom, odabrati neki $j$ sa $\overline{c_j}<0$.
...
$4 y_1 + y_2 \geq 1$ i $3 y_1 + y_2 \geq 1$. Da bi ocjena gornje granice bila što bolja u (\ref{eq:dual-1}), dakle koeficijent na desnoj strani što veći, potrebno je maksimizovati izraz $9y_1 + 8 y_2$, pod gore navedenim uslovima. Prema tome, početnom problemu, kojeg označavamo sa ($P$), odgovara dualni problem 
$$\begin{array}{cccc}
     \min                    &  9 y_1 &+& 8 y_2        \\
     {s.t. }     &  4 y_1 &+& y_2 \geq 1   \\
                             &  3 y_1 &+& y_2 \geq 1
...
\begin{proof}
         ...
         Primijenimo simpleks metod, pa posmatrajmo posljednju (optimalnu) simpleks tabelu:
