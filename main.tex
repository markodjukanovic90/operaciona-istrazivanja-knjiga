...
TODO...%https://faculty.math.illinois.edu/~mlavrov/docs/482-fall-2019/lecture19.pdf
...
uvijek izabere naredni dio (komponentu rješenja) koja naizgled donosi veće benefite nego odabir drugih komponenti. Ovi algoritmi se često koriste za rješavanje teških optimizacionih problema i najčešće su prva strategija koja se primjenjuje prije 
upotrebe složenijih paradigmi. Optimizacioni problem se rijetko može riješiti pomoću pohlepnih algoritma ali on mogu ponuditi dobra aproksimaciona rješenja i kratkom vremenu i često su osnov za konstrukciju složenijih algoritama pomoću kojih se generišu  rješenja boljeg kvaliteta. Napomenimo da pohlepni algoritmi ne mogu uvijek biti iskorišteni za rješavanje problema; recimo Frakcijski Knapsak problem se rješava ali ne i 0-1 Knapsack problem. 

U ovoj glavi razmatramo nekoliko najpoznatijih pohlepnih algoritama koji se nalaze u literaturi, kao što su:
\begin{itemize}
	\item  Kruskalov algoritam za minimalno pokrivajuće stablo (MST);
	\item  Primov algoritam za minimalno pokrivajuće stablo;
	\item  Dajkstra algoritam za nalaženje najkraćih puteva;
	\item  Gridi algoritam za problem najdužeg zajedničkog podniza za skup stringova.
\end{itemize}

Svaku težinu grane $uv$ u grafu $G$, označimo sa $w(uv)$. 
Krenimo prvo sa definicijom pokrivajućeg stabla, pa pritom i minimalnog pokrivajućeg stabla. Stablo $T =(V_1, E_1)$ je pokrivajuće za graf $G=(V,E)$ akko
$V_1 = V$ i $E_1 \subseteq E$. Pokirvajuće stablo $T$ grapfa $G$ sa minimalnom težinom 
$\sum_{e \in E(T)=E_1} w(e)$ se naziva minimalno pokrivajuće stablo (MST) grafa $G$. 
Postoji nekoliko efikasnih algoritama za pronalazak MST, koje u nastavku i opisujemo u detalje. 


\subsubsection{Primov algoritam za nalaženje minimalnog pokrivajućeg stabla.}


Algoritam izvršava sljedeće korake:

\begin{itemize}
	\item kreirati skup mstSkup da bi se pratili svi čvorovi koji su već uključeni u MST
	\item dodijeliti vrijednost svakom čvoru u ulaznom grafu. Prvo se te vrijednosti inicijalizuju na +$\infty$. Dodijeli vrijednosti 0 za čvor koji je izabran prvi u MST
	\item Dok god mstSkup ne uključi sve čvorove, raditi sljedeće:
	\begin{itemize}
		\item izabrati čvor $u$ koji nije u mstSkup sa minimalnom vrijednošću;
		\item mstSkup = mstSkup $\cup \{u\}$;
		\item Update vrijednosti svih susjeda od $u$ na sljedeći način. 
		Za svaki susjed od $v$, ako je težina grane $uv$ manja od
		nego prijašnja vrijednost od $v$, update vrijednost na $w(uv)$. 
	\end{itemize}
\end{itemize}

Ideja dodjele vrijednosti čvorovima koji još nisu prisutni u MST je u tome da se izabere grana minimalne težine koja treba da se doda u trenutno MST. Dakle, ove vrijednosti ukazuju na grane minimalne težine povezujući ih sa skupom čvorova koji su već uključeni u MST.

Kompleksnost algoritma. Lista susjedstva i pretraga: $O(|V|^2)$. Ako  koristimo prioritetni red za nalaženje grane minimalne težine, onda se postiže kompleksnost
od $O( |E| \log(|V|))$. 

MST ima aplikacije u (1) dizajniranju mreža kao što je telefonska, gdje je potrebno u svaku kuću (koja predstavlja čvor) postaviti linije tako da se povežu sve kuće a da se potroši što manje materijala; (2) aproksimaciji drugih teških optimizacionih problema kao što je problem trgovačkog putnika (TSP). To je problem u kojem je potrebno naći najkraći put koji posjećuje svaki čvor u (težinskom) grafu najviše jednom. Primijetimo da su takvi putevi takođe MST. Ali MST ne mora biti stablo, jer mogu postojati čvorovi koji su posjećeni više puta. U to slučaju izbacimo takve grane i dobijemo put.  Prema tome, težina MST je manja nego težina TSP, jer minimizujemo nad većim skupom rješenja; 
(3) Klaster analiza.
$k$-klaster problem  se može posmatrati kao problem nalaženja MST a potom brisanja $k-1$ najskupljih grana iz MST. 

\subsubsection{Kruskalov algoritam za nalaženje minimalnog pokrivajućeg stabla.}

Ovo je još jedan pohlepni algoritam za nalaženje MST. Radi na principu sortiranja 
grana na osnovu njihove težine. Preciznije, sljedeći koraci se izvršavaju:
...
\begin{enumerate}
	...
	grana doda u skup grana koje su do sada ubačene u MST; Ako to nije slučaj, ubaciti granu u trenutni MST. Inače, izbaciti grani iz daljeg razmatranja;
	\item Ponoviti korak 2, dok se ne doda tačno $|V|-1$ grana u MST.
\end{enumerate} 
...
se primjenjuje operacija nalaženja i dodavanja grane u skup. Ove dvije operacije se izvršavaju u $O(\log |V|)$ vremenu. Dakle, algoritam treba vrijeme od $O(|E|\log |E| + |E|\log |V|)$ za izvršavanje. Kako je $|E|=O(|V|^2)$, dobijamo traženu kompleksnost. 

\subsubsection{Dajkstrin algoritam za nalaženje najkraćih puteva.}

Ovaj algoritam je dosta sličan Primovom algoritmu. Ovaj algoritam generiše tzv. SPT (shortest path tree) sa datim korjenim čvorom tako da se formiraju dva skupa čvorova, jedan koji sadrži čvorove uključene u SPT, a na drugoj strani su ostali čvorovi. 
U svakom koraku, nalazimo čvor koji je u drugom skupu sa minimalnom udaljenšću od korjenog čvora. Zatim se posmatraju njegovi susjedi te težine datih grana. U slučaju da je zbirna vrijednost manja od trenutne najbolje težine susjeda, ta težina se ažurira.
Koraci algoritma su dati u Pseudokodu~\ref{alg:dijkstra-algorithm}.


\begin{algorithm} 
	\begin{algorithmic}[1]
		\FOR{ svaki cvor $v \in V$:} 	%// Inicijalizacija  
	...
	\end{algorithmic}
    		\caption{Dijkstra($G,s $)}\label{alg:dijkstra-algorithm}
\end{algorithm}

Vremenska kompleksnost gornje implementacije algoritma je $O(|V|^2)$. 

Ako je graf predstavljem listom susjedstva, i $Q$ je implementiran kao binarni hip, 
kompleksnost se redukuje na $O(|E| \cdot \log |V|)$.

Napomenimo da Dijkstrin algoritam ne radi sa grafovima koji imaju negativne težine (pogledati inicijane uslove u prethonom pseudokodu, te liniju 12).  Za ovakve grafove,  primjenjuje se Bellman–Ford algoritam, algoritam baziran na dinamičkom programiranju. Njega ne pominjemo ovdje.  %, o kojem ćemo nešto više reći u narednim sekcijama.

\subsubsection{Frakcioni Problem Ruksaka}

Problem je definisan na sljedeći način. Neka je dato $n$ proizvoda sa svojim težinama, 
te ruksak kapaciteta $W$. Zadatak je da se među svim proizvodima odaberu oni proizvodi i stave u ruksak tako da težina ruksaka bude što veća ali da se pri tom ne naruši kapacitet ruksaka.

U 0-1 Knapsack problemu, ne dopušta se dijeljenje proizvoda, ili se uzima cijeli (1) ili se uopšte ne uzima (0). U frakcionalnom Knapsack problemu svaki proizvod posjeduje težinu i količinu, te se kao takav može podijeliti na dijelove (recimo uzme se pola od ukupne količine proizvoda). Efektno rijesenje se dobije primjenom pohlepnog algoritma, na sljedeći način. Računajmo podjele količina/vrijednost, te se proizvodi  sortiraju na osnovu tih vrijednosti (količnik).  Zatim uzmemo proizvod sa najvećim količnikom i dodamo ga u ruksak. Nastavljamo proces dodavanja sa sljedećim proizvodom dok ga ne budemo mogli dodati u ruksak cijelog. Potom dodamo dio njega (onoliko koliko se može a da se kapacitet ruksaka ne naruši). 

\subsubsection{Pohlepni pristup za rješavanje LCS problema}

LCS problem kao ulaz uzima $n$ stringova (označimo ovaj skup sa $S$) proizvoljne dužine. Zadatak je da se nađe podniz makimalne dužine tako da je on zajednički podniz za sve stringove. Ovaj problem se može riješiti u $O(n^m)$ vremenu uz pomoć dinamičkog programiranja (biće riječi o tome u narednim sekcijama), gdje je $n$ dužina najdužeg stringa u ulazu, a $m$ broj stringova u ulazu.

Jedan od najjednostavnijih pristupa u rješavanu ovog problema je svakako \emph{pohlepni pristup}, tzv. \emph{Best-Next} (BN) heuristika. Prije nego navedemo detalje algoritma, uvedimo nekoliko definicija i oznaka. Za cjelobrojni vektor $\textrm{pL} = (\textrm{pL}_1,\ldots, \textrm{pL}_m)$, definišemo sa
$S[\textrm{pL}] = \{s_i[\textrm{pL}_i, |s_i| \mid i=1,\ldots,m\}$, gdje $|s|$ označava dužinu stringa $s$, a $s[x,y]$ podstring stringa $s$ koji počinje od karaktera na poziciji $x$, a završava sa karakterom na poziciji $y$. Dalje, sa $\textrm{pL}_{i,a}$ definišemo poziciju prvog pojavljivanja karaktera \texttt{a} koja je  $\textrm{pL}_{i,a} \geq \textrm{pL}_i$ (ukoliko karakter ne postoji, vracamo $n+1$). Skup karaktera $\Sigma_{\textrm{pL}}$ koji se pojavljuju u svakom od stringova u $S[\textrm{pL}]$ se nazivaju dopustive ekstenzije u odnosu na $\textrm{pL}$.

\noindent Pohlepni (BN) pristup izvršava sljedeće korake:
\begin{itemize}
	\item Kreće se od (parcijalnog) rješenja koje je prazna niska, $s=\varepsilon$;
	\item Pozicioniramo pointere $\textrm{pL} \gets (1,\ldots, 1) $ na karaktere sa samog početka riječi;
	\item Dok god jedan od $\textrm{pL}_i$ nije jednak $n+1$: 
	\item $\Sigma_{\textrm{pL}} \gets$ Naći dopustive ekstenzije u $S[\textrm{pL}]$
	\item Od svih karaktera u $\Sigma_{\textrm{pL}}$, odaberemo jedan koji minimizuje vrijednost funkcije 
	$$g(\textrm{pL},a) = \sum_{i=1}^m\frac{\textrm{pL}_{i,a} - \textrm{pL}_i + 1 }{|s_i| - \textrm{pL}_i + 1 }, a \in \Sigma_{\textrm{pL}}.$$
	Neka je taj karakter označen sa $a^*$. 
	\item Ažuriramo: $s \gets  s \cdot a^*$, $\textrm{pL}_i \gets \textrm{pL}_{i, a^*} +1, i = 1,\ldots,m.$
\end{itemize}
Algoritam vraća vrijednost $s$. 

Primijetimo da ovakav algoritam ne može da garantuje optimalno rješenje. 

\noindent Primjer.  Neka su data dva stringa: $s_1 = \texttt{acb}, s_2 = \texttt{bac}$. Ako uzmemo  $ b$ kao prvi karakter, rješenje koje dobijamo je \texttt{b}. Medjutim, optimalno rješenje je string $s=\texttt{ac}$. 

\subsection{Aproksimativni Algoritmi}
  %https://home.cs.colorado.edu/~srirams/courses/csci5654-fall13/ilpLectures.pdf
  
  %\section{Aproksimacioni Algoritmi}
  \begin{comment}
  Veliki broj optimizacionih problema u praksi jednostavno ne postoje efikasni algoritam, te je za  njihovo rjesavanje potrebano izdvojiti ogromne vremenske resurse. U osnovi, literatura ne poznaje algoritam koji rjesava ovakve problem a da se izvrsava u polinomijalnom vremenu $O(|P|)$, gdje je $P \in {Poly}$ neki polinom iz vektorskg prostora polinoma \emph{Poly}. U tu svrhu, \emph{aproksimacioni algoritmi} su razvijeni. 
  ...
  \subsection{Problem Pokrivanja Cvorova: aproksimacioni algoritmi}
  ...
  \begin{figure}
  	\centering
  	\includegraphics[width=60mm]{vertex-cover}
  	\caption{Primjer Pokrivaca cvora.} \label{vertex-cover}
  	\label{fig:case1}
  \end{figure}
  ...
  \begin{figure}
  	\centering
  	\includegraphics[width=120mm]{vc-2-example}
  	\caption{Graf.} \label{vertex-cover}
  	\label{fig:case1}
  \end{figure}
  ...
  \subsection{Problem Trgovackog Putnika: aproksimacioni algoritmi}
  
  Ovaj problem je tezak za aproksiaciju. Tome svjedoci sljedeca teorema.
  
  Ne postoji polinomijalan algoritam koji moze aproksimirati TSP sa faktorom $c > 1$, osim ako je $P = NP$. 
  
  Ako pretpostavimo da grane grafa zadovoljavaju nejednakost trougla, tj. za sve razlicite cvorove $u,v, r$ grafa vrijedi
  $$ w(u,v) + w(v,r) \geq w(u,r),$$
  gdje je $w(\cdot, \cdot)$ tezina grane koja spaja dva cvora. Ovaj problem je poznat 
  kao \emph{Metricki} TSP. On je i dalje NP tezak, ali se moze aproksimirati.  
  
  Posmatrajmo prvo \emph{algoritam najblizeg dodavanja} koji aproksimira TSP, dat Pseudokodom~\ref{alg:tsp-a1}. 
  
  %pseudokod algoritma 
  \begin{algorithm}[!t] 
  	\caption{Gridi algoritam za TSP.}\label{alg:tsp-a1}
  	\begin{algorithmic}[1]
  		\STATE \textbf{Ulaz}: kompletan graf $G$
  		\STATE \textbf{Izlaz}: Hamiltonova kontura $T$ sa najmanjom tezinom
  		\STATE $u,v \gets \mbox{argmin}_{uv \in E} w(u,v)$
  		\STATE $S \gets \{u,v\}$
  		\STATE $T \gets uv$
  		\While{$|S| == n$}
  		\STATE $u,t \gets$ $\mbox{argmin}_{(u,t) \in V\setminus T \times T} \{w(u, t)\}$
  		\STATE $ S \gets S \cup \{u\}$
  		\STATE Neka cvor $q$ slijedi $u$ u  $T$
  		\STATE Update $T$: $uq$ se izbacuje iz $T$, ubacuje se $utq$  
  		\EndWhile
  		\STATE \textbf{return} $T$
  	\end{algorithmic}
  \end{algorithm}
  
  Ovo je ocigledno polinomijalan algoritam. Ispitajmo koji aproksimacioni faktor 
  postize. Algoritam podsjeca na Primov algoritam. Grane koje se identifikuju algoritmom 
  tvore MST. Zakljucujemo da je cijena konture $T$ barem jednaka cijeni MST za iste ulazne parametre. Cijena konture za dva cvora $v_i$ i $v_j$ je $2 \cdot c_{v_i,v_j}$ (jer racunamo i povratnu granu). Posmatrajmo jednu iteraciju u Algoritmu~\ref{alg:tsp-a1}, gdje se $T$
  prosiruje za cvor $t$ koji se ubacuje izmedju $u$ i $q$. Povecanje cijene konture u tom slucaju je $$c_{u,t} + c_{t,q}-c_{u,q}.$$
  Na osnovu nejednakosti trougla imamo $ c_{t,q} \leq c_{t,u} + c_{u,q}$. Prema tome, porast cijene konture $T$ u svakoj iteraciji je najvise $ c_{t, u} + c_{u,t} = 2\cdot c_{u,t}$. 
  Prema tome, zavrsna tura ima cijenu koja dostize cijenu koja je barem dva puta veca od cijene MST. Prema tome, ovo je 2-faktor aproksimacioni algoritam. 
  
  \noindent Pokusajmo poboljsati faktor 2 aproksimacionog algoritma uz pomoc \emph{Double tree algoritma}. U tu svrhu, definisimo Ojlerovu konturu $T^o$ grafa $G$. To je kontura koja $e_1e_2...,e_me_1$ koja obilazi sve grane grafa $G$. Interesantno je da se takva kontura moze naci akko je $G$ je povezan te je svaki njegov cvor paran (takva klasa grafova se naziva Ojlerovi grafovi).
  
  %pseudokod algoritma 
  \begin{algorithm}[!t] 
  	\caption{Aproksimacioni algoritam 2 za TSP.}\label{alg:tsp-a2}
  	\begin{algorithmic}[1]
  		\STATE \textbf{Ulaz}: kompletan graf $G$
  		\STATE \textbf{Izlaz}: Hamiltonova kontura $T$ sa najmanjom tezinom
  		\STATE Izracunati MST
  		\STATE Zamijeniti svaku granu od MST sa dvije kopije istog
  		\STATE Naci Ojlerovu turu $T$
  		\STATE Skratiti turu $T$ da bi dobila Hamiltonov ciklus (konturu) $T$ (izbaciti duple cvorove na turi $T$) 
  		\STATE \textbf{return} $T$ 
  	\end{algorithmic}
  \end{algorithm}
  Ovaj algoritam osigurava 2--faktor aproksimaciju. 
  ...
  Pokazio da ovim algoritmom dobijamo 2-faktor aproksimaciju. Jasno je da je aloritam polinomijalan i da je dobijeno rjesenje dopustivo. Poksazimo sada da ostvarujemo faktor $2$. 
  Neka je $C^*$ cijena optimalne Hamiltonove konture. Tada je MST $\leq C^*$. 
  Lako se vidi da je cijena Ojlerove konture  $\leq 2 \cdot C^*$. Na osnovu nejednakosti trougla skracivanje Ojlerove konture da bi se dobila Hamiltonova kontura ne moze da uveca  cijenu dobijene Hamiltonove konture. Dakle, slijedi da je cijena Hamiltonove konture $\leq 2 C^*$, pa je ovaj algoritam 2--faktor aprokimacioni.
  
  Posmatrajmo sljedeci (aproksimacioni) algoritam koji umanjuje aproksimacioni faktor TSP sa $2$ na 1.5. Algoritam je poznat pod nazivom Kristofidesov algoritam~\ref{alg:tsp-a3}.
  
  %pseudokod algoritma 
  \begin{algorithm}[!t] 
  	\caption{Kristofidesov algoritam.}\label{alg:tsp-a3}
  	\begin{algorithmic}[1]
  		\STATE \textbf{Ulaz}: kompletan graf $G$
  		\STATE \textbf{Izlaz}: Hamiltonova kontura $T$ sa najmanjom tezinom
  		\STATE $T \gets$ Naci MST grafa $G$
  		\STATE $M \gets$ naci minimalan mecing svih cvorova u $T$ neparnog stepena
  		\STATE Dodamo grane $M$ u MST $T$
  		\STATE Nadjimo Ojlerovu konturu $T^o$
  		\STATE $T \gets$ Skratimo $T^o$ (duplikati cvorova se izbace)
  		\STATE \textbf{return} $T$
  	\end{algorithmic}
  \end{algorithm}
  
  Ovaj algoritam je polinomijalan i dobijeno rjesenje je dopustivo.
  Pokazimo da algoritam ima faktor 1.5.  Cijena TSP $C^*$ dostize najvise cijenu Ojlerove konture, tj.  $C^*\leq cost(T^o) = cost(T) + cost(M)$.  Znamo da je $cost(T) \leq C^*$. Pokazimo da je $cost(M) \leq  \frac{C^*}{2}$. Primijetimo da u optimalnoj turi $T$ postoje dva mecinga: prvi sa cvorovima iz $M$, a drugi za ostale cvorove (sa granam), oznacimo ga $M'$. 
  Kako je $C^* \geq cost(M) \leq cost(M') i cost(M) + cost(M') = cost(T)$, slijedi 
  $2 cost(M) \leq cost(T)$, odakle dobijamo tvrdjenje.
  ...
  \subsection{Problem Binarnog Pakovanja: aproksimacioni algoritmi}
  
  Definicija. Neka je dato $n$ proizvoda velicine $a_1,a_2,\ldots,a_n$ ($0<a_i<1$). Potrebno je spakovati sve proizvode u jediinicna pakovanja, tako da je broj pakovanja minimalan.  
  
  Primjer takvog pakovanja je dat na Slici~\ref{fig:bin-packing}.
  
  \begin{figure}
  	\centering
  	\includegraphics[width=120mm]{bin-packing}
  	\caption{Primjer Binarnog Pakovanja.} \label{fig:bin-packing}
  	\label{fig:case1}
  \end{figure}
  
  \emph{First-fit} algoritam. Ovo je gridi algoritam, koji se sastoji do sljedecih koraka:
  \begin{itemize}
  	\item stavljamo proizvode u redu u kojem su poredani za pakovanje, tj. $a_i$ se pakuje prije $a_j$ akko $i < j$;
  	\item stavljamo sljedeci proizvod u pakovanje sa najmanjim indeksom u koji moze da stane. \item ako ne odgovara niti jednom od otvorenih pakovanja, otvoriti novo pakovanje i smjestiti taj proizvod u to pakovanje.
  \end{itemize}
  
  \noindent Analiza algoritma. Ovo je polinomijalan algoritam koji vraca dopustivo rjesenje.  Neka je $C^*$ optimalan broj pakovanja koji se otvara da bi se spakovalo svih $n$ stavki. Pretpostavimo da algoritam vraca rjesenje $C$ (pakovanja). Iz toga je barem $C-1$ pakovanja puno vise od polovine. Odatle je
  $$C^* \geq \sum_{i=1}^n a_i > \frac{C-1}{2}\ \Rightarrow\  2 \cdot C^* > C-1 \Rightarrow 2 \cdot C^* \geq C,$$
  odakle slijedi da je algoritam 2--faktor aproksimacioni. 
  
  \emph{Best fit} algoritam. Sljedeci proizvod se stavlja u pakovanje koje ostavlja najmanju prazninu kada se prizvod smjesti u dato pakovanje. 
  Ako takvo pakovanje ne postoji, otvoriti novo pakovanje. 
  Ovaj algoritam je takodje 2--faktor aproksimacioni. Interesantno je napomenuti da za Problem Binarnog Pakovanja ne postoji aproksimacioni algoritam ciji je faktor $c <\frac{3}{2}$ (osim ako je P=NP). 
  
  \subsection{Problem Pokrivanja Skupa: aproksimacioni algoritmi}
  Ovaj problem je uopstenje Problema Pokrivanja Cvorova, i definisan je na sljedeci nacin.
  Ulaz problema je skup elemenata $X$ te familija podskupova $\mathcal{F}$ skupa $X$ tako da je za svaki $f\in \mathcal{F}$, $|f \cap X| \geq 1$ (tj. $\bigcup_{f \in \mathcal{F}} f = X$). 
  Potrebno je naci pokrivac $C \subseteq \mathcal{F}$ minimalne kardinalnosti tako da je 
  $$ \bigcup_{f \in C} f = X.$$
  
  U nastavku izlazemo gridi heuristiku koja predstavlja aproksimacioni algoritam sa logaritamskim faktorom. To znaci, kako velicina ulaza postaje veca, velicina aproksimacionog rjesenja raste relativno u odnosu na velicinu optimalnog rjesenja. Kako logaritam kao funkcija raste sporo, ovaj aproksimacioni algoritam daje korisne rezultate.
  
  %motivacija
  \begin{figure}
  	\centering
  	\includegraphics[width=70mm]{set-cover-1}
  	\caption{Primjer Pokrivanja Skupa ($\mathcal{F}=\{T_1,T_2,...,T_4\}$, rjesenje je skup $\mathcal{C}=\{T_3, T_4, T_1\}$} \label{fig:set-cover-1}
  	\label{fig:case1}
  \end{figure}
  
  Sto se tice motivacije za ovaj problem, pretpostavimo da imamo skup karakteristika, te skup ljudi koji posjeduju pojedine karakeristike. Potrebno je odrediti komisiju sa sto manjim brojem osoba tako da se za svaku karakteristiku moze naci osoba iz komisije da je posjeduje. Gridi algoritam za ovaj problem je dat Algoritmom~\ref{alg:set-cover-gridi}. 
  %pseudokod algoritma 
  \begin{algorithm}[!t] 
  	\caption{GREEDY-SET-COVER metod za Problem Pokrivanja Skupa.}\label{alg:set-cover-gridi}
  	\begin{algorithmic}[1]
  		\STATE \textbf{Ulaz}: $X$ i familija podskupova $\mathcal{F}$ od $X$
  		\STATE \textbf{Izlaz}: pokrivac skupa $X$
  		\STATE $U \gets X$
  		\STATE $C \gets \emptyset$
  		\While{$U \neq \emptyset$}
  ...
  $U$ predstavlja skup preostalih nepokrivenih elemenata. Skup $\mathcal{C}$ pedstavlja pokrivac koji se konstruise kroz iteracije algoritma. U svakom koraku, algoritam nastoji da prosiri pokrivac $\mathcal{C}$ na racun smanjenja skupa $U$. Kandidata za prosirivanje pokrivaca biramo na osnovu gridi funkcije datoj na liniji 6. U osnovi, biram onaj $f' \in \mathcal{F}$ koji ima najvise  elementima iz $U$ sa kojim ima neprazan presjek. Algoritam se izvodi u $O(|\mathcal{F}| \cdot |X|)$ vremenu, dakle koje je polinomijalno. Takodje, rjesenje koje se vraca algoritmom je dopustivo. Izracunajmo jos aproksimacioni faktor algoritma. Prije toga, oznacimo $d$-ti harmonijski broj $=H_d = \sum_{i=1}^d 1/i$ sa $H(d)$. 
  Vrijedi sljedeca teorema.
  
  \noindent \textbf{Teorema. } GREEDY-SET-COVER algoritam ima aproksimacioni faktor 
  $$ H(\max\{|f| \mid f \in \mathcal{F}\}).$$
  
  \noindent \textbf{Dokaz.} Oznacimo sa $f_i$ i-ti podskup koji je selektovan od strane algoritma: algoritam dodjeljuje cijenu $1$ kad se $ S_i$ doda u $\mathcal{C}$. 
  Cijenu selektovanja $f_i$ distribuiramo podjednakom vjerovatnocom na sve elemente koji su po prvi put pokriveni sa $f_i$. Posmatrajmo primjer dat na Slici~\ref{fig:set-cover-1}). U prvoj iteraciji gridi algoritma 
  6 elemenata je pokriveno sa $T_1$; prema tome, cijena svakog od ovih elemenata je $c_x = \frac{1}{|T_1\setminus \emptyset|}= \frac{1}{6}$. Dalje, u drugij iteraciji algoritam odabira skup $T_3$, koji pokriva 2 nova elementa; njihova cijena je $c_x=\frac{1}{|T_3 \setminus T_1|} = \frac{1}{2}$. U iteraciji broj 3, odabere se skup $T_2$, te se jedan novi element pokrije; njegova cijena je jednaka $c_x = \frac{1}{T_2 \setminus ( T_1 \cup T_3)}$. 
  
  Neka $ c_x$ oznacava cijenu za element $x\in X$.  Svakom elementu se cijena dodijeli najvise jednom, onog puta kada se pokrije po prvi put. Ako se $x$ pokriva prvi put sa $f_i$, tada imamo:
  $$c_x = \frac{1}{f_i \setminus (f_1 \cup ...\cup f_{i-1})}.$$
  
  Algoritam generise rjesenje $\mathcal{C}$, koje je cijene $|\mathcal{C}|$ i ova cijena se rasporedjuje preko elemenata iz $X$. Kako je i optimalni pokrivac $C^*$ takodje pokriva skup $X$, slijedi:
  \begin{align}\label{eq:f1}
  	|C| = \sum_{x \in X} c_x \leq \sum_{f \in C^*}\sum_{x \in f} c_x.
  \end{align}
  
  Pokazimo sljedecu kljucnu nejednakost koja sluzi u dokazu ove teoreme. Za svaku $f \in \mathcal{F}$, vrijedi  
  \begin{equation}\label{eq:f2}
  	\sum_{x \in f} c_x \leq H(|f|).
  \end{equation}
  
  Iz formula~(\ref{eq:f1}) i (\ref{eq:f2}) bi dobili da je 
  \begin{equation}
  ...
  Ostaje nam jos da pokazemo formulu~(\ref{eq:f2}). 
  
  Neka je $f\in \mathcal{F}$ tako da je $i=1,\ldots,|C|$, 
  $$u_i(f) = |f \setminus (f_1 \cup \cdots \cup f_{i} )|$$
  
  budu ostali elementi u $f$ koji su jos uvijek nepokriveni nakon sto je algoiritam odabarao skupove $f_1,\ldots, f_i$
  za pokrivac $C$ do $i$-te iteracije. Definisimo $u_0(f)=|f|$ ko broj elemenata skupa $f$ koji je inicijalno nepokriven. 
  Oznacimo sa $k$ kao posljednji indeks tako da je $u_k(f)=0$  --  to znaci da su svi elementi skupa $f$ u $k$-toj iteraciji
  pokriveni prvi put skupovima $S_1,\ldots,S_i$ koji su odabranu u algoritmu. Primijetimo da vrijedi
  $$ u_o(f) \geq u_1(f) \geq \ldots u_{k-1}(f) \geq u_k(f).$$
  Prema tome, $u_{i-1}(f) - u_{i}(f)$ predstavlja broj elemenata u supu $f$ koji su pokriveni 
  po prvi put sa $S_i$ u $i$-toj iteraciji za $i=1,\ldots,k$. Prema tome,
  
  \begin{align}\label{eq:q1}
  	\sum_{x \in f} c_x = \sum_{i=1}^k( u_{i-1}(f) - u_{i}(f)) \cdot \frac{1}{ f \setminus ( f_1 \cup \cdots \cup f_{i-1})},
  \end{align}
  
  \noindent sto je jednako sumi brojeva elemenata skupa $f$ pokriven sa $f_i$ pomnozeno sa cijenom koja je dodjeljena svakom elementu u $i$-toj iteraciji. Primijetimo da vrijedi:
  \begin{align}\label{eq:q2}
  	|f_i \setminus (f_1 \cup \cdots \cup f_{i-1})| \geq | f   \setminus (f_1 \cup \cdots \cup f_{i-1}) | = u_{i-1},
  \end{align}
  jer je $S_i$ gridi izbor u $i$-toj iteraciji i svaki drugi izbor ne moze pokriti vise novih (nepokrivenih do elemenata) od ovog skupa.
  
  Prema tome, iz jednakosti~(\ref{eq:f1}), te nejednakosti~(\ref{eq:f2}), slijedi 
  \begin{align*}
  	\sum_{x \in f} c_x \leq \sum_{i=1}^k ( u_{i-1} - u_{i}) \cdot \frac{1}{u_{i-1}}
  \end{align*}
  
  Izraz sa desne strane nejednakosti se moze ograniciti na sljedeci nacin:
  ...
  \begin{align}
  	...
  	&= \sum_{i=1}^k (H(u_{i-1}) - H(u_i)) = H(u_0)  H(u_k) = H(u_0) = H(|f|),
  \end{align}
  
  odakle slijedi tvrdjenje. 
  
  napomenimo da postoji i tezinska verzija Problema Pokrivanja Skupova. U ovoj verziji je svakom skupu iz $f \in\mathcal{F}$ data tezina $w_f$. Zadatak je naci pokrivac $C \subseteq \mathcal{F}$ skupa $X$ sa najmanjom tezinom. Ovaj problem na slican nacin kao i ne-tezinska varijanta moze biti aproksimirana do na faktor $H_d= \log d+O(1)$, gdje je $d= \max\{|f|:f \in \mathcal{F}\}$ uz pomoc slicnog gridi algoritma. U tom slucaju, gridi kriterij je odredjen funkcijom 
  \begin{align}
  	g(f) = \frac{w_f}{|f \cap U|}, f \in \mathcal{F}\setminus C,
  \end{align}
  gdje se u svakom koraku algoritma odredjuje skup $f^*$ koji minimizuje vrijednost funkcije $g$, koji se 
  zatim dodaje u trenutni pokrivac $C$.
  
  
  \subsection{Problem Najkraceg Nadstringa}
  Neka je dat skup stringova $S=\{s_1,...,s_n\}$. Zadatak ovog problema je da se 
  nadje string $s$ minimalne duzine koji sadrzi sve stringove iz $S$ kao njegove podstrigove. 
  
  Bez gubljenja opstosti, mozemo pretpostaviti da niti jedan string $s_i$ nije podstring stringa $s_j, i\neq j$.
  Ovaj prolem je NP--tezak. Ideja je rijesiti ovaj problem uz pomoc Problema Pokrivanja Skupa i 
  njegovih algoritama. Pokazacemo da postoji $2\cdot H_n$--faktor aproksimacija za problem najkraceg nadstringa. 
...
Upisivanje modela u .lp format se vrši pozivom funkcije \emph{writeLP}, čiji je argument naziv fajla. 
 ...
 \end{verbatim}
 \subsection{Primjeri modelovani preko rješavača}
